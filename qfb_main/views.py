        ...
        try:
            ...
            for article in articles:
                try:
                    ...
                    if request:
                        messages.success(
                            request, f"Article saved with ID: {news_article.id}")
            ...
            logger.error(
                f"API response missing 'articles' key: {response.text}")
        except Exception as e:
            tb_str = traceback.format_exception(type(e), e, e.__traceback__)
            tb_str = "".join(tb_str)
            logger.error(
                f"An error occurred while processing the API response: {e}\n{tb_str}")
    else:
        logger.error(
